...
export default function SimulationsPage() {
  ...
  const simulations = useMemo(() => tests.filter((t) => String(t.type).toLowerCase() === "simulation"), [tests]);
    ...
    <AppLayout>
      <div className="flex flex-col h-full bg-beige-100">
        <div className="bg-beige-100 border-b border-beige-300 px-8 py-6">
          ...
          <p className="text-brown-600 text-sm mt-1">Выберите симуляцию и потренируйтесь в формате чата</p>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {!simulations.length ? (
                <Card className="bg-white border border-beige-300 shadow-sm rounded-xl p-6">
                  ...
                  <div className="text-brown-600 text-sm mt-2">Добавьте симуляции через админку (тип: simulation).</div>
                </Card>
              ) : null}
            </div>
          )}
