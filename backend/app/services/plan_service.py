...
class PlanService:
    ...
    def _text_quality_score(self, value: str) -> int:
    ...
    def _repair_text_encoding(self, value: Any) -> str:
        ...
        for _ in range(2):
            ...
            for source_encoding in ("cp1251", "latin1"):
                ...
                if candidate == current:
                    continue
                if self._text_quality_score(candidate) > self._text_quality_score(current) + 3:
                    improved = candidate
                    break
            if improved is None:
                break
    ...
    def _repair_payload_encoding(self, value: Any) -> Any:
        if isinstance(value, str):
            return self._repair_text_encoding(value)
        if isinstance(value, list):
            return [self._repair_payload_encoding(item) for item in value]
        if isinstance(value, dict):
            return {key: self._repair_payload_encoding(item) for key, item in value.items()}
        return value
    ...
    def _normalize_text(self, value: Any) -> str:
        return self._repair_text_encoding(value).strip().lower()

    def _strip_final_marker(self, value: str) -> str:
        text = self._repair_text_encoding(value)
    ...
    def _final_test_title(self, target_difficulty: str) -> str:
        ...
        return f"Итоговый тест ({normalized})"

    def _legacy_final_test_title(self, target_difficulty: str) -> str:
        normalized = self._normalize_difficulty(target_difficulty)
        return f"[FINAL] Итоговый тест ({normalized})"

    def _final_simulation_title(self, target_difficulty: str) -> str:
        normalized = self._normalize_difficulty(target_difficulty)
        return f"Итоговая ролевая игра ({normalized})"

    def _legacy_final_simulation_title(self, target_difficulty: str) -> str:
        normalized = self._normalize_difficulty(target_difficulty)
        return f"[FINAL] Итоговая ролевая игра ({normalized})"

    def _final_test_description(self, target_difficulty: str) -> str:
        normalized = self._normalize_difficulty(target_difficulty)
        if normalized == "advanced":
            return "Финальная проверка уровня advanced: сложные управленческие решения, конфликт интересов и лидерство в условиях давления."
        if normalized == "intermediate":
            return "Финальная проверка уровня intermediate: координация команды, приоритизация и коммуникация с несколькими сторонами."
        return "Финальная проверка уровня beginner: базовая коммуникация, эмпатия и планирование в рабочих ситуациях."
    ...
    def _final_test_questions(self, target_difficulty: str) -> List[Dict[str, Any]]:
        ...
        if normalized == "advanced":
            return [
                {
                    "text": "На проекте одновременно конфликт в команде, риск срыва срока и запрос клиента на расширение объёма. Какой первый шаг сильнее всего?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Зафиксировать риски, провести короткую антикризисную встречу и согласовать план с ответственными"},
                        {"text": "Сразу пообещать клиенту всё выполнить без пересмотра сроков"},
                        {"text": "Передать проблему руководству и ждать решения"},
                    ],
                },
                {
                    "text": "Команда эмоционально выгорела, а бизнес давит на скорость. Что больше похоже на зрелое лидерство?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Сбалансировать нагрузку, прояснить приоритеты и запустить короткие циклы обратной связи"},
                        {"text": "Усилить контроль и отменить обсуждения"},
                        {"text": "Сфокусироваться только на KPI и игнорировать состояние команды"},
                    ],
                },
                {
                    "text": "Два стейкхолдера требуют взаимоисключающие решения. Как действовать?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Собрать критерии успеха, оценить компромиссы и предложить прозрачный вариант с последствиями"},
                        {"text": "Поддержать того, кто выше по должности"},
                        {"text": "Отложить решение до последнего дня"},
                    ],
                },
                {
                    "text": "Какой подход лучше всего демонстрирует критическое мышление?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Проверить допущения, данные и альтернативы перед решением"},
                        {"text": "Сразу выбрать знакомый вариант по опыту"},
                        {"text": "Сделать выбор по настроению команды"},
                    ],
                },
            ]
        if normalized == "intermediate":
            return [
                {
                    "text": "На еженедельной встрече возник спор, сроки под угрозой. Ваш первый шаг?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Остановить спор, уточнить факты и согласовать действия по срокам"},
                        {"text": "Выбрать одну сторону без обсуждения"},
                        {"text": "Перенести решение на следующую неделю"},
                    ],
                },
                {
                    "text": "Клиент просит срочную доработку, команда перегружена. Как корректно ответить?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Уточнить приоритет, показать влияние на план и предложить варианты"},
                        {"text": "Сразу согласиться без пересмотра плана"},
                        {"text": "Отказать без объяснений"},
                    ],
                },
                {
                    "text": "Вы ведёте две задачи с близкими дедлайнами. Что эффективнее?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Разбить задачи на этапы, назначить приоритеты и контрольные точки"},
                        {"text": "Работать только над более интересной задачей"},
                        {"text": "Ждать, пока приоритеты определит кто-то другой"},
                    ],
                },
                {
                    "text": "Сотрудник отвечает резко. Как лучше реагировать?",
                    "type": "multiple_choice",
                    "options": [
                        {"text": "Сохранять спокойствие, признать эмоции и вернуть разговор к фактам"},
                        {"text": "Ответить в том же тоне"},
                        {"text": "Прервать общение до конца дня"},
                    ],
                },
            ]
        return [
            {
                "text": "Коллега не понял задачу и раздражён. Ваш первый шаг?",
                "type": "multiple_choice",
                "options": [
                    {"text": "Спокойно уточнить цель и переформулировать задачу простыми шагами"},
                    {"text": "Сразу передать задачу другому человеку"},
                    {"text": "Игнорировать эмоции и требовать быстрый результат"},
                ],
            },
            {
                "text": "Как лучше распределить задачи на день?",
                "type": "multiple_choice",
                "options": [
                    {"text": "Определить приоритеты и оценить время на каждую задачу"},
                    {"text": "Начинать только с самых лёгких задач"},
                    {"text": "Делать задачи в случайном порядке"},
                ],
            },
            {
                "text": "Клиент просит изменения в последний момент. Как действовать?",
                "type": "multiple_choice",
                "options": [
                    {"text": "Уточнить требования, влияние на сроки и согласовать реалистичный вариант"},
                    {"text": "Сразу согласиться без уточнений"},
                    {"text": "Сразу отказаться без обсуждения"},
                ],
            },
            {
                "text": "В команде возникло напряжение. Что помогает снизить конфликт?",
                "type": "multiple_choice",
                "options": [
                    {"text": "Выслушать обе стороны и зафиксировать договорённости"},
                    {"text": "Назначить виновного без разбора ситуации"},
                    {"text": "Сделать вид, что проблемы нет"},
                ],
            },
        ]
    ...
    def _final_simulation_description(self, target_difficulty: str) -> str:
        normalized = self._normalize_difficulty(target_difficulty)
        if normalized == "advanced":
            return "Финальная ролевая игра уровня advanced: стрессовая ситуация с конфликтом интересов, лидерством и принятием решений в условиях высокой неопределённости."
        if normalized == "intermediate":
            return "Финальная ролевая игра уровня intermediate: управление коммуникацией между командой и заказчиком при риске срыва сроков."
        return "Финальная ролевая игра уровня beginner: проверка базовой коммуникации, эмпатии и тайм-менеджмента в рабочем диалоге."

    def _final_simulation_intro(self, target_difficulty: str) -> str:
        normalized = self._normalize_difficulty(target_difficulty)
        if normalized == "advanced":
            return (
                "Ситуация: вы руководите кросс-функциональным проектом, релиз через 48 часов. "
                "Технический лидер сообщает о критическом риске, бизнес требует не переносить дату, "
                "а клиент просит добавить новый функционал. "
                "В диалоге нужно удержать конструктивную коммуникацию, проявить эмоциональную устойчивость, "
                "оценить риски и предложить лидерский план действий с конкретными сроками и ответственными."
            )
        if normalized == "intermediate":
            return (
                "Ситуация: вы координируете рабочую группу, и за два дня до дедлайна одна ключевая задача отстаёт. "
                "Один сотрудник раздражён, второй молчит о рисках, заказчик просит подтвердить срок без переноса. "
                "В диалоге важно спокойно выстроить общение, уточнить причины задержки, расставить приоритеты "
                "и согласовать реалистичный план."
            )
        return (
            "Ситуация: вы отвечаете за небольшой рабочий блок. "
            "Перед дедлайном коллега не до конца понял задачу и нервничает, а клиент ждёт подтверждение результата в срок. "
            "В диалоге покажите эмпатию, уточните приоритеты и договоритесь о понятном плане действий."
        )
    ...
    def _is_final_title(self, value: str) -> bool:
        ...
        return (
            ...
            or "финальная проверка уровня" in normalized
            or "финальная ролевая игра уровня" in normalized
        )
    ...
    async def get_active_plan(
        ...
        plan = result.scalar_one_or_none()
        if plan is not None and isinstance(plan.content, dict):
            plan.content = self._repair_payload_encoding(plan.content)
    ...
    async def _generate_new_plan(
        ...
        try:
            ...
            plan_content = DevelopmentPlanContent(
                ...
                tasks=[
                    ...
                    TaskItem(
                        id="task_ct_1",
                        description="Перед решением задачи сформулируйте 5 уточняющих вопросов (про ограничения, риски, критерии успеха).",
                        skill="critical_thinking",
                        status="pending",
                        completed_at=None,
                    ),
                ...
                recommended_tests=[
                    TestRecommendation(
                        test_id=0,
                        title="Тест по мягким навыкам",
                        reason="Этот тест поможет оценить текущий уровень навыков и выбрать фокус в развитии.",
                    )
                ],
            )
        ...
        payload = self._repair_payload_encoding(plan_content.dict())
    ...
    async def _select_recommended_tests(
        ...
        def _resolve_skill(weakness: str) -> Optional[str]:
            if not weakness:
                return None
            normalized = self._normalize_text(weakness).replace("-", " ").replace("_", " ")
            for skill, keywords in skill_keywords.items():
        ...
        for t in tests:
            hay = f"{self._normalize_text(t.title)} {self._normalize_text(t.description)}"
            matched_skill: Optional[str] = None
            for skill, keywords in skill_keywords.items():
        ...
        return [
            TestRecommendation(
                test_id=int(t.id),
                title=self._repair_text_encoding(t.title),
                reason=reason,
            )
            for t in picked[:3]
        ]
